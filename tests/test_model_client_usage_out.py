"""TDD spec (premium#743, OSS seam half): ModelClient.chat() additive usage_out.

Design decision (Opus, 2026-07-16): the underlying vLLM/MLX calls already compute
...
``cached_tokens`` is structurally ``None`` for BOTH backends here: self-hosted
MLX/vLLM compute has no cache-read/write BILLING tier the way hosted Anthropic/
OpenAI APIs do (that's a separate, not-yet-tapped surface — see the metering
design doc's tap-point 1 note). Never estimate it; None is the honest value.
"""
