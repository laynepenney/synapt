"""TDD spec: OSS consolidation-stage envelopes.

One consolidate_stage UsageEvent per stage per cluster (B1-B4).
...
def test_run_extract_path_emits_one_consolidate_stage_event_per_stage(tmp_path):
    ...
    # Design intent follows the existing _run_extract_path sequence at
    # src/synapt/recall/consolidate.py:2807 and :2846-2872: B1, B2, B4, B3.
    ...
    infer_events = [e for e in received if e.op == "infer"]
    assert [e.detail for e in infer_events] == ["B1", "B2", "B4"]


def test_b4_retry_requests_are_distinguishable_from_the_first_attempt(tmp_path):
    from synapt.recall.consolidate import _rejoin_create_actions

    items = [
        {
            "action": "create",
            "content": f"meter retry witness {index}",
            "category": "fact",
            "tags": [],
            "source_turns": [],
    ...
    def invalid_infer(request):
        requests.append(request)
        return "not valid JSON"

    assert _rejoin_create_actions(
        items,
        "clu",
        invalid_infer,
        decision_log_path=tmp_path / "decisions.jsonl",
    ) == items
    assert requests[0]["stage"] == "B4"
    assert all(request["stage"] == "B4-retry" for request in requests[1:])
...
def test_consolidate_stage_events_never_disrupt_the_pipeline_on_a_broken_sink(tmp_path):
    ...
    assert any(event.op == "consolidate_stage" for event in invoked)
